...
def evidence_report():
 ...
 story+=[table(limits,[42*mm,55*mm,63*mm]),Spacer(1,5*mm),p('6. Conclusao tecnica','PigeH1'),p('A entrega constitui a arvore de release local 1.0.0 do PIGE360, com backend, contratos, migrations, SDK, interfaces-fonte, workflows, self-hosted e cadeia de fornecimento validados no que este host consegue executar. Builds que dependem de Docker, Rust, Android ou Xcode permanecem explicitamente nao executados neste host e devem ser produzidos pelos workflows em runners compativeis; nenhuma evidencia foi fabricada.')]
