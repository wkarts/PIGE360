...
const notifications = ref<Row[]>([]);
const people = ref<Row[]>([]);
const preferences = ref<Row[]>([]);
...
const selectedTemplate = computed(() =>
  templates.value.find((x) => x.id === sendForm.template_id),
);
const selectedVersion = computed(() =>
  selectedTemplate.value?.versions?.find(
    (x: Row) =>
      x.version === selectedTemplate.value?.current_version &&
      x.state === "published",
  ),
);
const variableNames = computed<string[]>(
  () => selectedVersion.value?.variables || [],
);
...
async function load() {
  ...
  try {
    const [t, n, p, pr] = await Promise.all([
      props.api.request<Row>("/communication/templates"),
      props.api.request<Row>("/notifications?limit=100"),
      props.api.request<Row>("/people"),
      props.api
        .request<Row>("/communication/preferences/me")
        .catch(() => ({ items: [] })),
    ]);
  ...
  <section class="communication-grid">
    <form class="panel" @submit.prevent="send">
      <div class="panel-title">
        ...
        <span>fila idempotente</span>
      </div>
      <label
        >Destinatário<select v-model="sendForm.recipient_person_id" required>
      ...
      ><label
        >Template<select v-model="sendForm.template_id">
          <option value="">Mensagem direta</option>
          <option
            v-for="t in templates.filter((x: Row) => x.state === 'published')"
            :key="t.id"
            :value="t.id"
          >
            {{ t.name }} · {{ t.channel }}
      ...
      ><label v-if="!selectedTemplate"
        >Canal<select v-model="sendForm.channel">
      ...
      ><template v-if="selectedTemplate"
        ><label v-for="v in variableNames" :key="v"
          >{{ v }}<input v-model="variables[v]" required /></label></template
      ><template v-else
        ><label>Assunto<input v-model="sendForm.subject" /></label
        ><label
          >Mensagem<textarea
      ...
      ><label
        >Agendar para<input
          ...
          type="datetime-local"
        /><small
          >Sem data: entra imediatamente no transactional outbox.</small
        ></label
      ><button class="primary" :disabled="loading">Enfileirar</button>
    </form>
    <form class="panel" @submit.prevent="createTemplate">
      <div class="panel-title">
        <h2>Novo template</h2>
      ...
      <label
        >Chave<input
          ...
          required /></label
      ><label>Nome<input v-model="templateForm.name" required /></label
      ><label
        >Canal<select v-model="templateForm.channel">
          <option>internal</option>
          <option>email</option>
          <option>whatsapp</option>
          <option>push</option>
      ...
      ><label
        ...
        ></textarea></label
      ><label
        >Variáveis declaradas<input
          v-model="templateForm.variables"
          placeholder="person.name, due_date" /></label
      ><button class="primary" :disabled="loading">Criar rascunho</button>
    </form>
  </section>
  <section class="panel">
    <div class="panel-title">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr v-if="!templates.length">
          <td colspan="5" class="empty">Nenhum template cadastrado.</td>
        </tr>
      </tbody>
    </table>
  </section>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="n in notifications" :key="n.id">
          <td>{{ dateBR(n.created_at) }}</td>
          <td>
            {{
              people.find((x: Row) => x.id === n.recipient_person_id)
                ?.full_name ||
          ...
          <td>{{ n.channel }}</td>
          <td>{{ n.subject || n.template_key || "Mensagem" }}</td>
          <td>
            <span
              class="pill"
              :class="
          ...
          <td class="row-actions">
            <button
              v-if="!['sent', 'cancelled', 'scheduled'].includes(n.state)"
              class="small"
              @click="retry(n)"
            >
              Retry</button
            ><button
              v-if="!['sent', 'cancelled'].includes(n.state)"
              class="small"
              @click="cancel(n)"
            >
              Cancelar
        ...
        <tr v-if="!notifications.length">
          <td colspan="7" class="empty">Nenhuma comunicação enfileirada.</td>
        </tr>
      </tbody>
    </table>
  </section>
  <section v-if="preferences.length" class="panel">
    <div class="panel-title"><h2>Minhas preferências</h2></div>
    <div class="preference-list">
      <span
        ...
        >{{ p.channel }} · {{ p.enabled ? "ativo" : "desativado" }}</span
      >
    </div>
  </section>
</template>
