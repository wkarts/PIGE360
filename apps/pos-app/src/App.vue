...
type CartLine=Row&{id:string;quantity:number;line:number};
...
const cartLines=computed<CartLine[]>(()=>products.value.filter(p=>(cart[p.id]||0)>0).map(p=>{const quantity=cart[p.id]??0;return {...p,id:String(p.id),quantity,line:Number(p.sale_price)*quantity} as CartLine;}));
