...
interface RuntimeConfiguration {
  hostname: string;
  institution: { id: string; legal_name: string; trade_name: string } | null;
  ...
  auth: { password_login: boolean; minimum_password_length: number; two_factor_available: boolean };
}
...
class ApiClient {
  ...
  get accessToken(): string | null { return sessionStorage.getItem("tenant.access_token"); }
  get refreshToken(): string | null { return sessionStorage.getItem("tenant.refresh_token"); }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      try { problem = await response.json() as ProblemDetail; } catch { /* resposta não estruturada */ }
...
interface ColumnConfig {
  key: string;
  label: string;
  format?: "date" | "datetime" | "money" | "number" | "boolean" | "minutes";
}

interface ActionConfig {
  label: string;
  method: HttpMethod;
  path: (row: RecordData) => string;
  openApiPath: string;
  schemaName?: string;
  preset?: RecordData;
  direct?: boolean;
  download?: boolean;
  visible?: (row: RecordData) => boolean;
}
...
interface ResourceConfig {
  ...
  listPath: string;
  createPath?: string;
  detailPath?: (row: RecordData) => string;
...
interface NavigationGroup { name: string; items: Array<{ id: string; icon: string; title: string }> }
...
interface DialogState {
  ...
  endpoint: string;
  method: HttpMethod;
  schema: RecordData | null;
  detail: RecordData | null;
  download: boolean;
}
...
const resources: ResourceConfig[] = [
  ...
  { id: "service-catalogs", group: "Serviços", icon: "▦", title: "Catálogos de serviços", description: "Catálogos por instituição e unidade, com vigência, estado e versionamento.", listPath: "/api/v1/service-catalogs", createPath: "/api/v1/service-catalogs", detailPath: (row) => `/api/v1/service-catalogs/${row.id}`, columns: [col("code"), col("name"), col("valid_from", undefined, "date"), col("valid_until", undefined, "date"), col("status")], statusParam: "status", actions: [
    { label: "Editar", method: "PATCH", path: (row) => `/api/v1/service-catalogs/${row.id}`, openApiPath: "/api/v1/service-catalogs/{catalog_id}", schemaName: "CatalogUpdate", preset: ({} as RecordData) }
  ] },
  { id: "services", group: "Serviços", icon: "◇", title: "Serviços", description: "Mensalidades, cursos, transporte, documentos e demais serviços recorrentes ou avulsos.", listPath: "/api/v1/services", createPath: "/api/v1/services", detailPath: (row) => `/api/v1/services/${row.id}`, columns: [col("code"), col("name"), col("service_type"), col("recurrence_type"), col("taxable", undefined, "boolean"), col("status")], statusParam: "status", actions: [
    { label: "Editar", method: "PATCH", path: (row) => `/api/v1/services/${row.id}`, openApiPath: "/api/v1/services/{service_id}", schemaName: "ServiceUpdate", preset: ({} as RecordData) },
...
function valueAt(row: RecordData, path: string): unknown {
  return path.split(".").reduce<unknown>((current, key) => current && typeof current === "object" ? (current as RecordData)[key] : undefined, row);
}

function statusClass(status: string): string {
  if (["active", "approved", "completed", "closed", "processed", "published", "paid", "healthy", "received"].includes(status)) return "success";
  if (["rejected", "cancelled", "failed", "terminated", "inactive"].includes(status)) return "danger";
  if (["draft", "pending", "open", "reopened", "scheduled", "not_configured", "partially_received"].includes(status)) return "warning";
  return "info";
}

function formatCell(value: unknown, format?: ColumnConfig["format"]): string {
...
createApp({
  setup() {
    ...
    const runtime = ref<RuntimeConfiguration | null>(null);
    ...
    const openApi = ref<RecordData | null>(null);
    ...
    const pageError = ref("");
    const dashboard = reactive<{ academic: RecordData | null; attendance: RecordData | null; finance: RecordData | null; services: RecordData | null; hr: RecordData | null; payroll: RecordData | null; timekeeping: RecordData | null }>({ academic: null, attendance: null, finance: null, services: null, hr: null, payroll: null, timekeeping: null });
    const toasts = ref<Array<{ id: string; type: ToastType; message: string }>>([]);
    const dialog = reactive<DialogState>({ open: false, mode: "details", eyebrow: "", title: "", submitLabel: "Salvar", fields: [], values: {}, error: "", loading: false, endpoint: "", method: "POST", schema: null, detail: null, download: false });
    ...
    const navigationGroups = computed<NavigationGroup[]>(() => {
      const grouped = new Map<string, NavigationGroup["items"]>();
    ...
    function applyTenantBranding(configuration: RuntimeConfiguration): void {
    ...
    async function login(): Promise<void> {
      ...
      authError.value = "";
      try {
        const tokens = await api.request<TokenResponse>("/api/v1/auth/login", {
          method: "POST",
    ...
    async function restoreSession(): Promise<void> {
      if (!api.accessToken && !api.refreshToken) return;
      try {
        const session = await api.request<{ user: UserData }>("/api/v1/auth/me");
    ...
    async function loadOpenApi(): Promise<void> {
      ...
      openApi.value = await api.request<RecordData>("/api/openapi.json", { retryAuthentication: false });
    }
    ...
    async function loadDashboard(): Promise<void> {
      ...
      try {
        const [academic, attendance, finance, servicesDashboard, hr, payroll, periods] = await Promise.all([
          api.request<RecordData>("/api/v1/academic/dashboard"),
          api.request<RecordData>("/api/v1/attendance/dashboard"),
          api.request<RecordData>("/api/v1/finance/dashboard"),
          api.request<RecordData>("/api/v1/services-dashboard"),
          api.request<RecordData>("/api/v1/hr/dashboard"),
          api.request<RecordData>("/api/v1/payroll/dashboard"),
          api.request<RecordData>("/api/v1/timekeeping/periods", { query: { limit: 5 } })
        ]);
    ...
    function navigate(route: string): void {
      ...
      else void reloadResource();
    }

    async function reloadResource(): Promise<void> {
      rows.value = [];
      nextCursor.value = null;
      await loadResource(false);
    }

    async function loadResource(append: boolean): Promise<void> {
      ...
      try {
        ...
        if (resource.searchParam && searchTerm.value) query[resource.searchParam] = searchTerm.value;
        if (resource.statusParam && statusFilter.value) query[resource.statusParam] = statusFilter.value;
        const result = await api.request<RecordData>(resource.listPath, { query });
    ...
    async function loadNextPage(): Promise<void> { if (nextCursor.value) await loadResource(true); }

    function resolveSchema(schema: RecordData | undefined): RecordData | null {
    ...
    function schemaByName(name: string): RecordData | null {
      return openApi.value?.components?.schemas?.[name] ?? null;
    }

    function operationBodySchema(path: string, method: HttpMethod): RecordData | null {
    ...
    function normalizePropertySchema(raw: RecordData): RecordData {
      if (raw.anyOf) {
        const meaningful = raw.anyOf.find((item: RecordData) => item.type !== "null") ?? raw.anyOf[0];
        return { ...meaningful, title: raw.title ?? meaningful?.title, default: raw.default ?? meaningful?.default };
      }
      return raw;
    }

    function fieldsFromSchema(schema: RecordData | null, preset: RecordData = {}): DynamicField[] {
      if (!schema) return [];
      ...
      return Object.entries(properties)
        ...
        .map(([name, raw]) => {
          const property = normalizePropertySchema(raw as RecordData);
          const type = String(property.type ?? "string");
          const kind: FieldKind = type === "integer" || type === "number" ? "number" : type === "boolean" ? "boolean" : type === "object" ? "object" : type === "array" ? "array" : "string";
          let inputType = "text";
          if (kind === "number") inputType = "number";
          ...
          if (name.includes("password")) inputType = "password";
    ...
    function initialValues(fields: DynamicField[], preset: RecordData = {}): RecordData {
      const values: RecordData = {};
      for (const field of fields) {
        if (Object.prototype.hasOwnProperty.call(preset, field.name)) values[field.name] = preset[field.name];
        else if (field.schema.const !== undefined) values[field.name] = field.schema.const;
        else if (field.schema.default !== undefined) values[field.name] = field.schema.default;
        else if (field.kind === "boolean") values[field.name] = false;
        else if (field.kind === "object") values[field.name] = "{}";
        else if (field.kind === "array") values[field.name] = field.schema.items?.type === "object" ? "[]" : "";
        else values[field.name] = "";
      }
      return values;
    }

    function openForm(title: string, eyebrow: string, endpoint: string, method: HttpMethod, schema: RecordData | null, preset: RecordData = {}, submitLabel = "Salvar"): void {
    ...
    async function openDetails(row: RecordData): Promise<void> {
      const resource = activeResource.value;
      dialog.open = true;
      dialog.mode = "details";
      ...
      if (resource?.detailPath) {
        dialog.loading = true;
        try { dialog.detail = await api.request<RecordData>(resource.detailPath(row)); }
        catch (error) { dialog.error = errorMessage(error); }
    ...
    function convertFieldValue(field: DynamicField, value: unknown): unknown {
      ...
      if (field.kind === "object") {
        ...
        catch { throw new Error(`${field.label}: informe um objeto JSON válido.`); }
      }
      if (field.kind === "array") {
        ...
        if (!text) return [];
        if (text.startsWith("[")) {
          try { return JSON.parse(text); } catch { throw new Error(`${field.label}: informe uma lista JSON válida.`); }
        }
        return text.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean);
      }
      if (field.inputType === "datetime-local" && typeof value === "string" && value) return new Date(value).toISOString();
      return value;
    }

    function formPayload(): RecordData {
      ...
      if (dialog.schema?.properties?.institution_id && user.value?.institution_id) payload.institution_id = user.value.institution_id;
      if (dialog.schema?.properties?.unit_id && user.value?.unit_id) payload.unit_id = user.value.unit_id;
      return payload;
    }
    ...
    async function submitDialog(): Promise<void> {
      ...
      try {
        ...
        if (currentRoute.value === "dashboard") await loadDashboard();
      } catch (error) {
    ...
    function visibleActions(row: RecordData): ActionConfig[] {
      return (activeResource.value?.actions ?? []).filter((action) => !action.visible || action.visible(row));
    }

    async function executeConfiguredAction(action: ActionConfig, row: RecordData): Promise<void> {
      if (action.download) {
        try {
          const blob = await api.request<Blob>(action.path(row), { method: action.method, responseType: "blob" });
    ...
    function handleHashChange(): void {
      ...
      if (currentRoute.value === "dashboard") void loadDashboard();
      else void reloadResource();
    }
    ...
    onMounted(async () => {
      ...
      try {
        runtime.value = await api.request<RuntimeConfiguration>("/api/v1/auth/runtime", { retryAuthentication: false });
