...
<template>
  ...
  <div v-else class="app-shell" data-surface="tenant">
    ...
    <main>
      ...
      <template v-else-if="active === 'academic_progress'"
      ...
      <template v-else-if="active === 'procurement'"
        ><ProcurementPanel
          :api="api"
          @error="error = $event"
          @notice="notice = $event"
      /></template>
      <template v-else-if="active === 'assets'"
        ><AssetsPanel
          :api="api"
          @error="error = $event"
          @notice="notice = $event"
                  ...
                  integrationForm.name =
                    integrationForm.provider === 'cloudflare'
                      ...
                      : integrationForm.provider === 'mailcow'
                        ? 'Mail institucional'
                        : integrationForm.provider === 'evolution'
                          ? 'WhatsApp Evolution'
                          : integrationForm.provider.toUpperCase()
                "
              >
                <option value="cloudflare">Cloudflare</option>
                <option value="mailcow">Mailcow</option>
                <option value="evolution">Evolution API</option>
                <option value="inep">INEP / Educacenso</option>
      ...
      <template v-else-if="active === 'compliance'"
        ><CompliancePanel
          :api="api"
          @error="error = $event"
          @notice="notice = $event" /></template
      ><template v-else-if="active === 'government_education'"
        ><GovernmentEducationPanel
          :api="api"
          @error="error = $event"
          @notice="notice = $event"
      /></template>
