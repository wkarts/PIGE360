...
const quotationSuppliers = computed(
  () => selectedQuotation.value?.suppliers ?? [],
);
...
async function load(): Promise<void> {
  ...
  try {
    const [
      ...
      request<Row>("/suppliers?limit=200"),
      request<Row>("/products?limit=300"),
      request<Row>("/procurement/requisitions?limit=200"),
      request<Row>("/procurement/quotations?limit=200"),
      request<Row>("/procurement/orders?limit=200"),
      request<Row>("/inventory/lots?limit=300"),
      request<Row>("/inventory/reservations?limit=300"),
      request<Row>("/inventory/reorder-policies"),
      request<Row>("/inventory/purchase-suggestions"),
    ]);
...
async function createBarcode(): Promise<void> {
  try {
    await post(
      "/inventory/product-barcodes",
      { ...barcodeForm, variant_id: nullable(barcodeForm.variant_id) },
      idempotency("product-barcode"),
    );
...
async function showRequisition(row: Row): Promise<void> {
  try {
    selectedRequisition.value = await request<Row>(
      `/procurement/requisitions/${row.id}`,
    );
...
async function requisitionAction(
  ...
  try {
    ...
    const body =
      action === "approve"
        ? {
            approved_quantities: Object.fromEntries(
              (detail.items ?? []).map((item: Row) => [
                item.id,
    ...
    await load();
    if (
      selectedRequisition.value?.requisition?.id === row.id ||
...
async function showQuotation(row: Row): Promise<void> {
  try {
    selectedQuotation.value = await request<Row>(
      `/procurement/quotations/${row.id}`,
    );
...
async function submitProposal(): Promise<void> {
  ...
  try {
    await post(
      `/procurement/quotations/${selectedQuotation.value.quotation?.id ?? selectedQuotation.value.id}/suppliers/${proposalForm.supplier_id}/proposal`,
      {
        ...
        items: quotationItems.value.map((item: Row) => ({
    ...
    await showQuotation(
      selectedQuotation.value.quotation ?? selectedQuotation.value,
    );
    await load();
  } catch (error) {
    emit("error", message(error));
...
async function awardQuotation(): Promise<void> {
  ...
  try {
    const result = await post<Row>(
      `/procurement/quotations/${selectedQuotation.value.quotation?.id ?? selectedQuotation.value.id}/award`,
      awardForm,
      idempotency("quotation-award"),
    );
...
async function receiveOrder(): Promise<void> {
  ...
  try {
    await post(
      `/procurement/orders/${selectedOrder.value.order?.id ?? selectedOrder.value.id}/receipts`,
      {
        supplier_document_number: nullable(
          receiptForm.supplier_document_number,
        ),
        items: [
          {
    ...
    emit(
      "notice",
      "Recebimento registrado com estoque e custo médio atualizados.",
    );
    await load();
...
async function returnOrderItem(): Promise<void> {
  ...
  try {
    await post(
      `/procurement/orders/${selectedOrder.value.order?.id ?? selectedOrder.value.id}/returns`,
      {
...
async function createCount(): Promise<void> {
  try {
    activeCount.value = await post<Row>(
      "/inventory/counts",
      {
  ...
  try {
    ...
    activeCount.value = await post<Row>(
      `/inventory/counts/${countId}/complete`,
      {
        ...
        items: (activeCount.value.items ?? []).map((item: Row) => ({
          item_id: item.id,
...
async function generatePurchaseSuggestions(): Promise<void> {
  try {
    const result = await post<Row>(
      "/inventory/purchase-suggestions/generate",
      {},
      idempotency("purchase-suggestions"),
    );
    const summary = result.summary ?? {};
    emit(
      "notice",
      `Sugestões processadas: ${summary.created ?? 0} novas, ${summary.refreshed ?? 0} atualizadas e ${summary.superseded ?? 0} encerradas.`,
    );
...
async function convertSelectedSuggestion(): Promise<void> {
  ...
  try {
    const result = await post<Row>(
      `/inventory/purchase-suggestions/${selectedSuggestion.value.id}/convert`,
      {
    ...
    emit(
      "notice",
      `Sugestão convertida na requisição ${result.requisition?.requisition_number ?? result.requisition?.id}.`,
    );
    selectedSuggestion.value = null;
    await load();
  } catch (error) {
...
async function dismissSelectedSuggestion(): Promise<void> {
  ...
  try {
    await post(
      `/inventory/purchase-suggestions/${selectedSuggestion.value.id}/dismiss`,
      {
        expected_version: selectedSuggestion.value.version,
    ...
    emit(
      "notice",
      "Sugestão descartada com justificativa e trilha de auditoria.",
    );
    selectedSuggestion.value = null;
    await load();
  } catch (error) {
  ...
  <div class="procurement-module">
    <section class="metrics">
      <article>
        ...
        ><strong>{{
          suppliers.filter((row: Row) => row.status === "active").length
        }}</strong
        ><small>cadastro homologado</small>
      </article>
      <article>
        <span>Requisições abertas</span
        ><strong>{{
          requisitions.filter(
            (row: Row) =>
              !["cancelled", "rejected", "converted"].includes(row.status),
          ).length
        }}</strong
        ><small>fluxo de aprovação</small>
      </article>
      <article>
        <span>Cotações</span><strong>{{ quotations.length }}</strong
        ><small>propostas e adjudicação</small>
      </article>
      <article>
        <span>Pedidos pendentes</span
        ><strong>{{
          orders.filter(
            (row: Row) => !["received", "cancelled"].includes(row.status),
          ).length
        }}</strong
        ><small>recebimento parcial ou integral</small>
      </article>
      <article>
        <span>Lotes ativos</span
        ><strong>{{
          lots.filter((row: Row) => Number(row.quantity) > 0).length
        }}</strong
        ><small>validade e rastreabilidade</small>
      </article>
      <article>
        <span>Sugestões abertas</span
        ><strong>{{
          purchaseSuggestions.filter((row: Row) => row.status === "open").length
        }}</strong
        ><small>reposição por estoque mínimo</small>
      </article>
    </section>
    <section class="procurement-tabs">
      <button
        :class="{ selected: tab === 'suppliers' }"
        @click="tab = 'suppliers'"
      >
        Fornecedores e produtos</button
      ><button
        :class="{ selected: tab === 'requisitions' }"
        @click="tab = 'requisitions'"
      >
        Requisições</button
      ><button
        :class="{ selected: tab === 'quotations' }"
        @click="tab = 'quotations'"
      >
        Cotações</button
      ><button :class="{ selected: tab === 'orders' }" @click="tab = 'orders'">
        Pedidos e recebimentos</button
      ><button
        :class="{ selected: tab === 'inventory' }"
        @click="tab = 'inventory'"
      >
        Lotes, reservas e inventário</button
      ><button
        :class="{ selected: tab === 'reorder' }"
        @click="tab = 'reorder'"
      >
        Estoque mínimo e reposição</button
    ...
    <template v-if="tab === 'suppliers'">
      <section class="grid-2 forms">
        <form class="panel" @submit.prevent="createSupplier">
          <h2>Novo fornecedor</h2>
          <div class="cols">
            <label>Código<input v-model="supplierForm.code" /></label
            ><label
              >CNPJ<input v-model="supplierForm.cnpj" inputmode="numeric"
            /></label>
          </div>
          <label
            >Razão social<input
              v-model="supplierForm.legal_name"
              required /></label
          ><label
            >Nome fantasia<input v-model="supplierForm.trade_name"
          /></label>
          <div class="cols">
            <label
              >E-mail<input v-model="supplierForm.email" type="email" /></label
            ><label>Telefone<input v-model="supplierForm.phone" /></label>
          </div>
          <div class="cols">
            <label
              >Contato principal<input
                v-model="supplierForm.contact_name" /></label
            ><label
              >E-mail do contato<input
                v-model="supplierForm.contact_email"
                type="email"
            /></label>
          </div>
          <button class="primary">Cadastrar fornecedor</button>
        </form>
        <div class="panel">
          <h2>Garantias do fluxo</h2>
          <ul class="checklist">
            <li>
              Idempotência em cadastros, requisições, pedidos e recebimentos.
            </li>
            <li>Recebimento parcial com bloqueio de quantidade excedente.</li>
            <li>Lote obrigatório quando definido no perfil do produto.</li>
            <li>
              Entrada, custo médio, devolução e reserva na mesma transação.
            </li>
            <li>Auditoria e outbox em todas as transições.</li>
          </ul>
        </div>
      </section>
      <section class="grid-2 forms">
        ...
        <form class="panel" @submit.prevent="createVariant">
          ...
          <label
            >Produto<select v-model="variantForm.product_id" required>
          ...
          >
          <div class="cols">
            <label>SKU<input v-model="variantForm.sku" required /></label
            ><label>Nome<input v-model="variantForm.name" required /></label>
          </div>
          <div class="cols">
            <label
              >Custo<input
          ...
          <button class="primary">Cadastrar variação</button>
        </form>
        <form class="panel" @submit.prevent="createBarcode">
          <h2>Código de barras</h2>
          <label
            >Produto<select v-model="barcodeForm.product_id" required>
              <option v-for="row in products" :key="row.id" :value="row.id">
                {{ row.name }}
              </option>
            </select></label
          >
          <div class="cols">
            <label>Código<input v-model="barcodeForm.barcode" required /></label
            ><label
              >Tipo<select v-model="barcodeForm.barcode_type">
          ...
          <label class="inline"
            ><input v-model="barcodeForm.primary" type="checkbox" /> Código
            principal</label
          ><button class="primary">Cadastrar código</button>
        </form>
      </section>
      <section class="panel">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="row in suppliers" :key="row.id">
              ...
              <td>{{ row.cnpj || "—" }}</td>
              <td>{{ row.email || row.contacts?.[0]?.email || "—" }}</td>
              <td>{{ row.rating ?? "—" }}</td>
              <td>{{ row.status }}</td>
              <td>
                <button class="small" @click="toggleSupplier(row)">
                  {{ row.status === "active" ? "Inativar" : "Ativar" }}
                </button>
              </td>
            </tr>
            <tr v-if="!suppliers.length">
              <td colspan="7" class="empty">Nenhum fornecedor cadastrado.</td>
            </tr>
          </tbody>
        </table>
      </section>
    ...
    <template v-else-if="tab === 'requisitions'">
      <section class="grid-2 forms">
        <form class="panel" @submit.prevent="createRequisition">
          <h2>Nova requisição</h2>
          <label
            >Produto<select v-model="requisitionForm.product_id" required>
              <option v-for="row in products" :key="row.id" :value="row.id">
                {{ row.name }}
              </option>
            </select></label
          ...
          <div class="cols">
            <label
              >Quantidade<input
                ...
                required /></label
            ><label
              >Preço estimado<input
                v-model="requisitionForm.estimated_unit_price"
                type="number"
                min="0"
                step="0.01"
            /></label>
          ...
          <label
            >Necessário até<input
              v-model="requisitionForm.needed_by"
              type="date" /></label
          ><label
            >Justificativa<textarea
              v-model="requisitionForm.justification"
              rows="4"
              required
            ></textarea></label
          ><button class="primary">Criar requisição</button>
        </form>
        <div class="panel" v-if="selectedRequisition">
          <h2>Detalhes da requisição</h2>
          <p>
            <strong>{{
              selectedRequisition.requisition?.number ||
              selectedRequisition.number
            }}</strong>
            ·
            {{
              selectedRequisition.requisition?.status ||
              selectedRequisition.status
            }}
          </p>
          <div class="rows">
            <div v-for="item in selectedRequisition.items || []" :key="item.id">
              <div>
                <strong>{{ item.product_name || item.product_id }}</strong
                ><small
                  >solicitado {{ item.requested_quantity || item.quantity }} ·
                  aprovado {{ item.approved_quantity || 0 }}</small
                >
              </div>
            </div>
          </div>
        ...
        <div v-else class="panel">
          ...
          <p>
            Selecione uma requisição para consultar itens, quantidades aprovadas
        ...
        <table>
          ...
          <tbody>
            <tr v-for="row in requisitions" :key="row.id">
              ...
              <td>{{ row.status }}</td>
              <td>
                <button class="small" @click="showRequisition(row)">
                  Detalhes</button
                ><button
                  v-if="row.status === 'draft'"
                  class="small"
                  @click="requisitionAction(row, 'submit')"
                >
                  Enviar</button
                ><button
                  v-if="row.status === 'submitted'"
                  class="small"
                  @click="requisitionAction(row, 'approve')"
                >
                  Aprovar</button
                ><button
                  v-if="row.status === 'submitted'"
                  class="small"
                  @click="requisitionAction(row, 'reject')"
                >
                  Rejeitar</button
                ><button
                  v-if="
                  ...
                  "
                  class="small"
                  @click="requisitionAction(row, 'cancel')"
                >
                  Cancelar
                </button>
              </td>
    ...
    <template v-else-if="tab === 'quotations'">
      <section class="grid-2 forms">
        <form class="panel" @submit.prevent="createQuotation">
          <h2>Nova cotação</h2>
          <label
            >Requisição aprovada<select
              v-model="quotationForm.requisition_id"
              required
            >
              <option value="">Selecione</option>
              <option
                v-for="row in requisitions.filter(
                  (item: Row) => item.status === 'approved',
                )"
                :key="row.id"
                :value="row.id"
              >
                {{ row.number }} · {{ row.justification }}
              </option>
            </select></label
          ><label
            >Fornecedor convidado<select
              v-model="quotationForm.supplier_id"
              required
            >
              <option
                v-for="row in suppliers.filter(
          ...
          ><label
            >Prazo de resposta<input
              v-model="quotationForm.response_deadline"
              type="datetime-local" /></label
          ><button class="primary">Criar cotação</button>
        </form>
        <form class="panel" @submit.prevent="submitProposal">
          <h2>Registrar proposta</h2>
          <label
            >Fornecedor<select v-model="proposalForm.supplier_id" required>
              <option
                v-for="row in quotationSuppliers"
                :key="row.supplier_id"
          ...
          <div class="cols">
            <label
              >Preço unitário<input
                v-model="proposalForm.unit_price"
                type="number"
                min="0"
                step="0.01"
                required /></label
            ><label
              >Quantidade disponível<input
                v-model="proposalForm.quantity_available"
                type="number"
                min="0.0001"
                step="0.0001"
                required
          ...
          <div class="cols">
            <label
              >Prazo (dias)<input
                v-model.number="proposalForm.delivery_days"
                type="number"
                min="0" /></label
            ><label
              >Pagamento (dias)<input
                v-model="proposalForm.payment_days"
                placeholder="30,60"
            /></label>
          </div>
          <label>Marca<input v-model="proposalForm.brand" /></label
          ><button class="primary" :disabled="!selectedQuotation">
            Registrar proposta
          </button>
        </form>
      </section>
      <section v-if="selectedQuotation" class="grid-2 forms">
        <div class="panel">
          <h2>Itens da cotação</h2>
          <div class="rows">
            <div v-for="row in quotationItems" :key="row.id">
              <div>
                <strong>{{ row.product_name || row.product_id }}</strong
                ><small>quantidade {{ row.quantity }}</small>
              </div>
            </div>
          </div>
        </div>
        <form class="panel" @submit.prevent="awardQuotation">
          <h2>Adjudicar</h2>
          <label
            >Fornecedor vencedor<select
              v-model="awardForm.supplier_id"
              required
            >
              <option
                v-for="row in quotationSuppliers.filter(
                  (item: Row) => item.status === 'responded',
                )"
                :key="row.supplier_id"
                :value="row.supplier_id"
              >
          ...
          <div class="cols">
            <label
              >Depósito<input
                v-model="awardForm.warehouse_id"
                required /></label
            ><label
              >Entrega esperada<input
                v-model="awardForm.expected_on"
                type="date"
            /></label>
          </div>
          <div class="cols">
            <label
              >Frete<input
                v-model="awardForm.freight_amount"
                type="number"
                min="0"
                step="0.01" /></label
            ><label
              >Desconto<input
                v-model="awardForm.discount_amount"
                type="number"
                min="0"
                step="0.01"
            /></label>
          ...
          <label
            >Justificativa<textarea
              ...
              required
            ></textarea></label
          ><button class="primary">Adjudicar e criar pedido</button>
        </form>
      </section>
      <section class="panel">
        <div class="panel-title">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="row in quotations" :key="row.id">
              <td>{{ row.number }}</td>
              <td>{{ row.requisition_number || row.requisition_id || "—" }}</td>
              <td>{{ row.response_deadline || "—" }}</td>
              <td>{{ row.supplier_count ?? row.suppliers?.length ?? 0 }}</td>
              <td>{{ row.status }}</td>
              <td>
                <button class="small" @click="showQuotation(row)">
                  Detalhes
                </button>
              </td>
            </tr>
    ...
    <template v-else-if="tab === 'orders'">
      <section class="grid-2 forms">
        <form class="panel" @submit.prevent="createOrder">
          <h2>Novo pedido direto</h2>
          <label
            >Fornecedor<select v-model="orderForm.supplier_id" required>
              <option
                v-for="row in suppliers.filter(
                  (item: Row) => item.status === 'active',
                )"
          ...
          ><label
            >Produto<select v-model="orderForm.product_id" required>
              <option v-for="row in products" :key="row.id" :value="row.id">
                {{ row.name }}
              </option>
            </select></label
          ...
          <div class="cols">
            ...
            ><label
              >Entrega esperada<input
                v-model="orderForm.expected_on"
                type="date"
            /></label>
          </div>
          <button class="primary">Criar pedido</button>
        </form>
        <div class="panel" v-if="selectedOrder">
          <h2>
            Pedido {{ selectedOrder.order?.number || selectedOrder.number }}
          </h2>
          <p>
            Estado:
            <strong>{{
              selectedOrder.order?.status || selectedOrder.status
            }}</strong>
          </p>
          <div class="rows">
            <div v-for="item in selectedOrderItems" :key="item.id">
              <div>
                <strong>{{ item.product_name || item.product_id }}</strong
                ><small
                  >pedido {{ item.quantity }} · recebido
                  {{ item.received_quantity || 0 }} · devolvido
                  {{ item.returned_quantity || 0 }}</small
                >
              </div>
            </div>
          </div>
        ...
        <div v-else class="panel">
          <h2>Recebimento e devolução</h2>
          <p>
            Selecione um pedido para registrar notas, lotes, quantidades
            recebidas e devoluções.
          </p>
        </div>
      </section>
      <section v-if="selectedOrder" class="grid-2 forms">
        <form class="panel" @submit.prevent="receiveOrder">
          <h2>Registrar recebimento</h2>
          <label
            >Item<select v-model="receiptForm.purchase_order_item_id" required>
              <option
                v-for="row in selectedOrderItems"
                :key="row.id"
                :value="row.id"
              >
          ...
          <label
            >Documento do fornecedor<input
              v-model="receiptForm.supplier_document_number"
          /></label>
          <div class="cols">
            <label>Lote<input v-model="receiptForm.lot_number" /></label
            ><label
              >Validade<input v-model="receiptForm.expires_on" type="date"
            /></label>
          </div>
          <button class="primary">Confirmar recebimento</button>
        </form>
        <form class="panel" @submit.prevent="returnOrderItem">
          <h2>Devolução ao fornecedor</h2>
          <label
            >Item<select v-model="returnForm.purchase_order_item_id" required>
              <option
                v-for="row in selectedOrderItems"
                :key="row.id"
                :value="row.id"
          ...
          <div class="cols">
            <label
              >Quantidade<input
                ...
                required /></label
            ><label
              >Lote<select v-model="returnForm.lot_id">
                <option value="">Sem lote</option>
                <option v-for="row in lots" :key="row.id" :value="row.id">
                  {{ row.lot_number }} · {{ row.product_name }}
                </option>
              </select></label
            >
          </div>
          <label
            >Motivo<textarea
              ...
              required
            ></textarea></label
          ><button class="primary">Registrar devolução</button>
        </form>
      </section>
      <section class="panel">
        <div class="panel-title">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="row in orders" :key="row.id">
              ...
              <td>{{ row.received_percentage ?? 0 }}%</td>
              <td>{{ row.status }}</td>
              <td>
                <button class="small" @click="showOrder(row)">Detalhes</button
                ><button
                  v-if="row.status === 'draft'"
                  class="small"
    ...
    <template v-else-if="tab === 'inventory'">
      <section class="grid-2 forms">
        <form class="panel" @submit.prevent="createReservation">
          <h2>Reserva de estoque</h2>
          <label
            >Produto<select v-model="reservationForm.product_id" required>
              <option v-for="row in products" :key="row.id" :value="row.id">
                {{ row.name }}
              </option>
            </select></label
          ><label
            >Lote<select v-model="reservationForm.lot_id">
              <option value="">Sem lote específico</option>
              <option
                v-for="row in lots.filter(
                  (item: Row) => item.product_id === reservationForm.product_id,
                )"
                :key="row.id"
                :value="row.id"
              >
                {{ row.lot_number }} · saldo {{ row.quantity }}
              </option>
            </select></label
          >
          <div class="cols">
            <label
              >Quantidade<input
                ...
                required /></label
            ><label
              >Depósito<input v-model="reservationForm.warehouse_id" required
            /></label>
          </div>
          <div class="cols">
            <label
              >Origem<input
                v-model="reservationForm.source_type"
                required /></label
            ><label
              >Identificador<input v-model="reservationForm.source_id" required
            /></label>
          </div>
          <button class="primary">Reservar</button>
        </form>
        <form class="panel" @submit.prevent="createCount">
          <h2>Novo inventário</h2>
          <label
            >Depósito<input v-model="countForm.warehouse_id" required /></label
          ><label
            >Produto<select v-model="countForm.product_id">
          ...
          ><label class="inline"
            ><input v-model="countForm.include_zero_balance" type="checkbox" />
            Incluir saldo zero</label
          ><button class="primary">Abrir inventário</button>
        </form>
      </section>
      <section v-if="activeCount" class="panel">
        <div class="panel-title">
          <h2>Contagem em andamento</h2>
          <span>{{ activeCount.count?.number || activeCount.number }}</span>
        </div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="row in activeCount.items || []" :key="row.id">
              <td>{{ row.product_name || row.product_id }}</td>
              <td>{{ row.expected_quantity }}</td>
              <td>
                <input
                  v-model="countLines[row.id]"
                  type="number"
                  min="0"
                  step="0.0001"
                />
        ...
        <button
          v-if="
            (activeCount.count?.status || activeCount.status) !== 'completed'
          "
          class="primary"
          @click="completeCount"
        >
          Concluir inventário e ajustar
        </button>
      </section>
      <section class="grid-2">
        <div class="panel">
          ...
          <table>
            ...
            <tbody>
              <tr v-for="row in lots" :key="row.id">
                <td>{{ row.product_name || row.product_id }}</td>
              ...
              <tr v-for="row in reservations" :key="row.id">
                ...
                <td>
                  <button
                    ...
                    class="small"
                    @click="reservationAction(row, 'consume')"
                  >
                    Consumir</button
                  ><button
                    v-if="row.status === 'active'"
                    class="small"
                    @click="reservationAction(row, 'release')"
                  >
                    Liberar
                  </button>
                </td>
    ...
    <template v-else>
      <section class="grid-2 forms">
        <form class="panel" @submit.prevent="saveReorderPolicy">
          <div class="panel-title">
            ...
            <span v-if="editingPolicy">v{{ editingPolicy.version }}</span>
          </div>
          <label
            >Produto<select
              v-model="reorderForm.product_id"
          ...
          <label
            >Depósito<input
              v-model="reorderForm.warehouse_id"
              :disabled="!!editingPolicy"
              required
          /></label>
          <div class="cols">
            <label
              >Estoque mínimo<input
                v-model="reorderForm.minimum_quantity"
                type="number"
                min="0.0001"
                step="0.0001"
                required /></label
            ><label
              >Estoque alvo<input
                v-model="reorderForm.target_quantity"
                type="number"
                min="0.0001"
                step="0.0001"
                required
          ...
          <div class="cols">
            <label
              >Prazo de reposição (dias)<input
                v-model.number="reorderForm.lead_time_days"
                type="number"
                min="0"
                max="3650" /></label
            ><label
              >Fornecedor preferencial<select
                v-model="reorderForm.preferred_supplier_id"
              >
          ...
          <div class="actions">
            <button class="primary">
              {{
                editingPolicy ? "Salvar alterações" : "Cadastrar política"
              }}</button
            ><button
              v-if="editingPolicy"
              type="button"
              class="small"
              @click="clearPolicyEditor"
            >
              Cancelar edição
            </button>
          </div>
        ...
        <div class="panel">
          ...
          <p>
            O cálculo considera saldo físico, reservas ativas e quantidades
            ainda pendentes em pedidos aprovados. Sugestões abertas são
            atualizadas sem duplicidade e encerradas quando a necessidade deixa
            de existir.
          </p>
          <ul class="checklist">
            <li>Limites por produto e depósito.</li>
            <li>Fornecedor preferencial e prazo de reposição.</li>
            <li>Conversão transacional em requisição de compra.</li>
            <li>Idempotência, concorrência otimista, auditoria e outbox.</li>
            <li>
              Aplicável à cantina, materiais, uniformes, livros e demais
              estoques.
            </li>
          </ul>
          <button
            ...
            :disabled="
              loading ||
              !reorderPolicies.some((row: Row) => row.status === 'active')
            "
            @click="generatePurchaseSuggestions"
          >
            Recalcular sugestões de compra
          </button>
        </div>
      </section>
      ...
      <section class="panel">
        <div class="panel-title">
          <h2>Políticas de estoque mínimo</h2>
          <span>{{ reorderPolicies.length }}</span>
        </div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="row in reorderPolicies" :key="row.id">
              <td>
                <strong>{{ row.product_name }}</strong
                ><small>{{ row.product_sku }}</small>
              </td>
              <td>{{ row.warehouse_id || row.warehouse }}</td>
              <td>{{ row.stock?.available_quantity ?? 0 }}</td>
              <td>{{ row.stock?.open_purchase_quantity ?? 0 }}</td>
              <td>{{ row.stock?.projected_quantity ?? 0 }}</td>
              <td>{{ row.minimum_quantity }} / {{ row.target_quantity }}</td>
              <td>{{ row.preferred_supplier_name || "—" }}</td>
              <td>{{ row.status }}</td>
              <td>
                <button class="small" @click="editReorderPolicy(row)">
                  Editar</button
                ><button class="small" @click="toggleReorderPolicy(row)">
                  {{ row.status === "active" ? "Inativar" : "Ativar" }}
                </button>
              </td>
            </tr>
            <tr v-if="!reorderPolicies.length">
              <td colspan="9" class="empty">Nenhuma política cadastrada.</td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <section class="grid-2">
        <div class="panel">
          <div class="panel-title">
            ...
            <span>{{ purchaseSuggestions.length }}</span>
          </div>
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="row in purchaseSuggestions" :key="row.id">
                <td>
                  <strong>{{ row.product_name }}</strong
                  ><small>{{ row.warehouse_id }}</small>
                </td>
                <td>
                ...
                <td>
                  ...
                  >
                    Analisar</button
                  ><span v-else-if="row.requisition_id"
                    >Req. {{ row.requisition_id.slice(-8) }}</span
                  ><span v-else>{{ row.closure_reason || "—" }}</span>
                </td>
              </tr>
              <tr v-if="!purchaseSuggestions.length">
                <td colspan="6" class="empty">Nenhuma sugestão calculada.</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div v-if="selectedSuggestion" class="panel">
          ...
          <p>
            <strong>{{ selectedSuggestion.product_name }}</strong>
          </p>
          <div class="rows">
            <div>
              <div>
                <span>Saldo projetado</span
                ><small>disponível + compras aprovadas</small>
              </div>
              <strong>{{ selectedSuggestion.projected_quantity }}</strong>
            </div>
            <div>
              <div>
                <span>Limite mínimo</span><small>gatilho configurado</small>
              </div>
              <strong>{{ selectedSuggestion.minimum_quantity }}</strong>
            </div>
            <div>
              <div>
                <span>Quantidade sugerida</span
                ><small>até o estoque alvo</small>
              </div>
              <strong>{{ selectedSuggestion.suggested_quantity }}</strong>
            </div>
          </div>
          <form @submit.prevent="convertSelectedSuggestion">
            <label
              >Necessário até<input
                v-model="suggestionActionForm.needed_by"
                type="date" /></label
            ><label
              >Justificativa<textarea
                v-model="suggestionActionForm.justification"
                rows="3"
                required
              ></textarea></label
            ><button class="primary">Converter em requisição</button>
          </form>
          <form
            class="dismiss-form"
            @submit.prevent="dismissSelectedSuggestion"
          >
            <label
              >Motivo do descarte<textarea
                v-model="suggestionActionForm.reason"
                rows="3"
                required
              ></textarea></label
            ><button class="small">Descartar sugestão</button>
          </form>
        </div>
        <div v-else class="panel">
          <h2>Análise da sugestão</h2>
          <p>
            Selecione uma sugestão aberta para convertê-la em requisição de
            compra ou descartá-la com justificativa auditada.
          </p>
        </div>
      </section>
    </template>
...
.procurement-tabs button.selected {
  border-color: var(--brand-primary);
  box-shadow: 0 0 0 1px var(--brand-primary);
}
.procurement-tabs .refresh {
  margin-left: auto;
}
.rows > div {
  display: flex;
