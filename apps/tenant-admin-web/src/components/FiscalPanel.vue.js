...
const assemblyForm = reactive({
    ...
    items_json: '[{"line_id":"1","item_kind":"product","code":"ITEM","description":"Item fiscal","quantity":"1","unit_price":"0.00","discount":"0","total_amount":"0.00","classification":{}}]',
});
...
const awaitingProvider = computed(() => documents.value.filter((row) => row.provider_status === "not_configured")
    .length);
const filteredUnits = computed(() => contextForm.institution_id
    ...
    : units.value);
const importSources = computed(() => catalogSources.value.filter((row) => !catalogImportForm.catalog_id ||
...
function sourceProfilesLabel(row) {
    const sources = Array.isArray(row.source_profiles)
        ...
        : [];
    return (sources
        .map((source) => `${String(source.provider_key ?? source.provider_type ?? "fonte")}@${String(source.provider_version ?? "1")}`)
        .join(", ") || "—");
}
...
async function createCatalog() {
    try {
        const created = await post("/fiscal/catalogs", {
...
async function createCatalogVersion() {
    ...
    try {
        ...
        await post(`/fiscal/catalogs/${catalogVersionForm.catalog_id}/versions/${version.id}/publish`, {
            expected_version: version.version,
...
async function importCatalogFile() {
    if (!catalogImportForm.catalog_id ||
    ...
    try {
        ...
        emit("notice", result.state === "quarantined"
...
async function publishCatalogImport(run) {
    try {
        ...
        await post(`/fiscal/catalog-imports/${run.id}/publish`, {
...
async function rollbackCatalogImport(run) {
    if (!run.catalog_version_id)
        return;
    try {
        await post(`/fiscal/catalogs/${run.fiscal_catalog_id}/versions/${run.catalog_version_id}/rollback`, {
...
async function createClassification() {
    try {
        ...
        await post(`/fiscal/classification-rules/${created.id}/publish`, {
...
function removeTaxComponent(index) {
    if (taxComponents.value.length > 1)
...
async function createTaxRuleVersion() {
    ...
    try {
        ...
        const components = taxComponents.value.map((component) => ({
            ...
            monophase_amount_per_unit: nullable(String(component.monophase_amount_per_unit || "")),
        ...
        await post(`/fiscal/tax-rule-sets/${ruleSet.id}/versions/${created.id}/publish`, {
...
async function createLegalSource() {
    try {
        await post("/fiscal/legal-sources", {
            ...legalSourceForm,
...
async function createStrategy() {
    try {
        ...
        await post("/fiscal/strategy-rules", {
            ...strategyForm,
...
async function createRtcSchedule() {
    try {
        await post("/fiscal/rtc-schedules", {
            ...rtcForm,
...
async function loadIbptOffline() {
    try {
...
async function rollbackIbpt(snapshot) {
    try {
...
async function createDeliveryPolicy() {
    try {
        const result = await post("/fiscal/delivery-policies", {
            ...
            contingency_after_attempts: deliveryPolicyForm.contingency_after_attempts || null,
...
async function renderFiscalDocument(row) {
    try {
        const result = await post(`/fiscal/documents/${row.id}/render`, {
            force: false,
        });
...
async function substituteFiscalDocument(row) {
    ...
    if (!reason)
        return;
    try {
        await post(`/fiscal/documents/${row.id}/substitute`, {
            source_type: "manual",
...
async function requestCorrectionEvent(row) {
    ...
    if (!text)
        return;
    try {
        await post(`/fiscal/documents/${row.id}/events`, {
...
async function createDocumentSchema() {
    try {
        const result = await post("/fiscal/document-schemas", {
...
async function createRoutingPolicy() {
    try {
        ...
        const result = await post("/fiscal/routing-policies", {
            ...
            product_document_type: nullable(routingPolicyForm.product_document_type),
...
async function assembleFiscalDocument() {
    try {
        ...
        assemblyResult.value = await post("/fiscal/document-assemblies", {
        ...
        emit("notice", assemblyResult.value.state === "blocked_validation"
    ...
    if (__VLS_ctx.tab === 'contexts') {
        ...
        if (__VLS_ctx.selectedContext) {
            ...
            for (const [version] of __VLS_getVForSourceType((__VLS_ctx.selectedContext.versions ?? []))) {
                ...
                __VLS_elementAsFunction(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({
                    ...{ class: ("pill") },
                    ...{ class: ((version.status === 'published'
                            ? 'ok'
        ...
        for (const [kind] of __VLS_getVForSourceType(([
            'NCM',
            'NBS',
            'LC116',
            'CFOP',
        ...
        for (const [row] of __VLS_getVForSourceType((__VLS_ctx.catalogs))) {
            ...
            __VLS_elementAsFunction(__VLS_intrinsicElements.td, __VLS_intrinsicElements.td)({});
            (row.active_version
                ? __VLS_ctx.formatDate(row.active_version.valid_from)
                : "—");
            __VLS_elementAsFunction(__VLS_intrinsicElements.td, __VLS_intrinsicElements.td)({});
            __VLS_elementAsFunction(__VLS_intrinsicElements.code, __VLS_intrinsicElements.code)({});
        ...
        for (const [component, index] of __VLS_getVForSourceType((__VLS_ctx.taxComponents))) {
            ...
            for (const [tax] of __VLS_getVForSourceType(([
                'ICMS',
                'ICMS_ST',
                'FCP',
                'IPI',
            ...
            for (const [incidence] of __VLS_getVForSourceType(([
                'taxable',
                'exempt',
                'deferred',
                'suspended',
        ...
        for (const [uf] of __VLS_getVForSourceType(([
            'AC',
            'AL',
            'AP',
            'AM',
        ...
        if (__VLS_ctx.documentTransparency) {
            ...
            __VLS_elementAsFunction(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({});
            __VLS_elementAsFunction(__VLS_intrinsicElements.b, __VLS_intrinsicElements.b)({});
            (__VLS_ctx.documentTransparency.ibpt_provider_profile_id ??
                "cache local legado");
            __VLS_elementAsFunction(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({});
            __VLS_elementAsFunction(__VLS_intrinsicElements.b, __VLS_intrinsicElements.b)({});
            __VLS_elementAsFunction(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({});
